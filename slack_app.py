    ...
    try:
        ...
        if update_title != paper_title:
            ...
            response = client.chat_postMessage(
                channel="C06EGTS2M1T",  # Replace with your channel
                text=text_message
            )
...
while True:
    ...
    with open('scholars_publications.json', 'r') as file:
        scholars_publications = json.load(file)
    for scholar, paper_title in scholars_publications.items():
        try:
            update, update_title = get_latest_article(scholar)
        except:
            print(f"\nCouldn't find the google scholar profile for {scholar}.\n")
            continue
        if update_title != paper_title:
            print(update)
            name = scholar.split(',')[0]
            text_message = f":rolled_up_newspaper::test_tube: {name} has an update on Google Scholar! \
                ```Title: {update['title']}\nCitation: {update['citation']}\nYear: {update['pub_year']}```"
            response = client.chat_postMessage(
                channel="C06EGTS2M1T",   # C06QE7RGZ5J
                text=text_message
            )
            scholars_publications[scholar] = update_title
